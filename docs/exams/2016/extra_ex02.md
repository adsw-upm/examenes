...
??? note "Mostrar solución"
    ...
    ```
    O(N) porque se trata de un bucle que recorre los elementos del array y, dentro del bucle, las operaciones contains() y add() son O(1).
    
    Soluciones alternativas con otra complejidad son:
    ```java
    // O(n log n)
