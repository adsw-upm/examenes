...
Se quiere desarrollar una clase monitora `TorreControl` para gestionar una única pista de aterrizaje de un aeropuerto, con acceso exclusivo por parte de objetos de la clase `Avion`, que serán las hebras. Los aviones pueden pedir a la torre de control aterrizar y despegar. Se pide desarrollar los siguientes métodos:

![](./p1r/p1r_ex01.png)

- (a) (1,5 puntos) `public ... void avisoAterrizaje(Avion unAvion)`: Para la maniobra de aterrizaje, los aviones primero avisarán a la torre de control (con este método) de su intención de aterrizar y después preguntarán por el permiso para poder aterrizar. Cuando la torre de control reciba un aviso de aterrizaje deberá añadir el avión que solicitó el permiso a una lista. Esta lista se ordenará por el combustible que tengan los aviones, siendo el que tenga menos combustible el primero de la lista y el primero en aterrizar (en caso de haber dos aviones con el mismo combustible será indiferente el orden de aterrizaje entre ellos).

- (b) (2 puntos) `public ... void permisoAterrizaje(Avion unAvion)`: Un avión invoca este método para pedir permiso para aterrizar. Cuando la torre de control reciba la llamada de permiso de aterrizaje entonces solo dejará aterrizar al avión si lo tiene guardado en la lista de menos combustible y si la pista está libre. 

- (c) (1,5 puntos) `public ... void permisoDespegue(Avion unAvion)`: Para la maniobra de despegue los aviones pedirán permiso para despegar. Se supone que todos los aviones tienen el máximo de combustible y no hay prioridad, así que cuando pidan despegar podrán hacerlo siempre y cuando la pista est# libre.
