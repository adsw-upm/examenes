...
- (b) (1 punto) Suponga que tenemos un gestor de eventos táctiles onTouch(View v, MotionEvent event). Si pulsamos en el objetivo (círculo azul situado en el extremo superior derecho), ¿qué valores nos darán x e y si se calculan como sigue? int x = (int) event.getX() / lado1; int y = (int) event.getY() / lado1
...
Deseamos cambiar el comportamiento del gestor de eventos táctiles, de forma que el jugador ya no se mueva con el dedo. Queremos que al poner el dedo en una posición, aparezca un Patito si la casilla está vacía. El id de Patito registrado en Juego es 0. Además, queremos que muestre:
    - una traza informativa (info) al pulsar en la pantalla. Ej. “Pulso en (3, 4)”
    - una traza de depuración (debug) cuando pone el patito. Ej. “Patito puesto en (3, 4)”
    - una traza de depuración si no coincide la posición al poner y quitar el dedo, porque ha realizado algún movimiento. Ej. “No coinciden poner y quitar dedo”
    - una traza de advertencia (warn) si ya hay algún móvil en esa posición. “Patito no puesto ya hay otro móvil”.
