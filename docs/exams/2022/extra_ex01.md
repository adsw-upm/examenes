...
Se propone un prototipo de un sistema de ficheros simplificado, que solo permite acceder a ficheros a partir de su nombre, y almacenar nuevos ficheros en el sistema. Habrá cuatro clases en el sistema: 1) Fichero; 2) Gestor, que será la clase principal; 3) Disco, que contendrá un conjunto de ficheros; 4) Cache, para almacenar ficheros que han sido utilizados recientemente y evitar volver a buscarlos. La relación entre estas clases se ve en el diagrama adjunto.
...
??? note "Mostrar solución"
    Las operaciones en Gestor tendrán una complejidad:
        - get() en general es O(t_get_cache + t_get_disco * d + t_put_cache)
        - get() de un fichero reciente es O(t_get_cache)
        - put() es O(t_put_cache + t_put_disco)
