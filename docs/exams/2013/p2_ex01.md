...
Se proporcionan las clases Vino y Bodega (ver al final). 
...
objeto ```Vino``` en los siguientes componentes: 
    ...
    - 1. foto en un ```ImageView``` (*R.id.foto*),
    - 2. nombre, denominación y descripción en tres ```EditText``` (*R.id.nombre*, *R.id.denominacion*, *R.id.descripcion*).
