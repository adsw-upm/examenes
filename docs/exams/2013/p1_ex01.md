...
- (1,5 puntos) a) Dados un array y el desplazamiento \( h \), desarrolle un algoritmo de complejidad O(1) que devuelva el número mayor conociendo el array y el desplazamiento:
```java
...
- (1,5 puntos) b) Desarrolle un algoritmo de complejidad O(n) que devuelva el número mayor conociendo el array, pero sin saber el desplazamiento:
```java
...
- (2 puntos) c) Desarrolle un algoritmo de complejidad O(log n) que devuelva el número mayor conociendo el array, pero sin conocer el desplazamiento:
```java
...
??? note "Mostrar solución"
    ...
    - Tras k pasos, el rango es N/2<sup>k</sup>; de forma que el rango se reduce a 1 (h encontrado) cuando 1 <= N/2<sup>k</sup> -> k=log(N)1 <= N / 2^k  ➜  k = log₂(N) ⊂ O(log N)
