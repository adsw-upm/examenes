...
Se quiere simular un sistema de impresión con una impresora a la que varios usuarios pueden pedir impresiones.
Los usuarios pueden pedir imprimir un texto (una cadena de caracteres).
La impresora debe imprimir un texto solo cuando no esté ocupada, y siempre debe dar prioridad al texto más corto.
Los usuarios se modelan como hebras independientes.

Se proporciona el siguiente diagrama de clases como referencia, y se pide:

```{=latex}

\begin{minipage}[thp]{12cm}
...
- (b) (2 puntos) Implementa una clase `Impresora` que contenga un método `imprimir` que asegure que:
    * Solo se permite imprimir un texto en cada momento
    ...
    * Se debe mostrar por consola un mensaje en los siguientes momentos: 1) cuando un usuario intenta imprimir; 2) tras imprimir cada carácter; y 3) cuando se ha terminado de imprimir un texto completo. Los mensajes deben contener el nombre de la hebra que ha realizado la acción.
